...
// GET /stats/game — общая статистика игры
  ...
  try {
    const { rows: totals } = await db.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM plates) AS total_plates,
    ...
    // Новые игроки по дням (последние 30 дней)
...
// GET /stats/players — список всех игроков с монетами
  ...
  try {
    const { rows } = await db.query(`
      SELECT
        ...
        p.coins,
        p.created_at,
        COUNT(pl.id) AS plates_count,
        COUNT(CASE WHEN pl.status='listed' THEN 1 END) AS listed_count
      FROM players p
