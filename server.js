...
app.post('/market/buy', async (req, res) => {
  ...
  try {
    ...
    if (parseInt(invRows[0].count) >= 20) { await client.query('ROLLBACK'); return res.status(400).json({ ok: false, error: 'Инвентарь покупателя полон (макс 10)' }); }
    await client.query('UPDATE players SET coins=coins-$1, updated_at=NOW() WHERE telegram_id=$2', [plate.listed_price, buyer_id]);
    const sellerAmount = plate.listed_price; // без комиссии — продавец получает полную сумму
