...
// POST /admin/remove_upgrade — отобрать улучшение из upg_inventory игрока
// Body: { admin_id, target_id, upgrade_key }
app.post('/admin/remove_upgrade', requireAdmin, async (req, res) => {
  try {
    ...
    const upgInventory = rows[0].upg_inventory || [];
    // Отбираем первый незадействованный экземпляр данного ключа
    const idx = upgInventory.findIndex(u => u.lotKey === upgrade_key && !u.applied);
    if (idx === -1) return res.status(404).json({ ok: false, error: 'Улучшение не найдено или уже применено' });

    const [removed] = upgInventory.splice(idx, 1);
