...
async function initDB() {
  ...
  await db.query(`ALTER TABLE players ADD COLUMN IF NOT EXISTS inventory JSONB DEFAULT '[]'`).catch(()=>{});
  await db.query(`ALTER TABLE players ADD COLUMN IF NOT EXISTS last_seen TIMESTAMP`).catch(()=>{});
...
// POST /player/inventory/sync — сохранить текущий инвентарь игрока
app.post('/player/inventory/sync', async (req, res) => {
  try {
    const { telegram_id, inventory } = req.body;
    if (!telegram_id) return res.status(400).json({ ok: false, error: 'Нет telegram_id' });
    await db.query(
      `UPDATE players SET inventory=$1, last_seen=NOW(), updated_at=NOW() WHERE telegram_id=$2`,
      [JSON.stringify(inventory || []), telegram_id]
    );
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
app.get('/stats/player/:telegram_id', async (req, res) => {
  ...
  try {
    ...
    // Инвентарь — берём из колонки players.inventory (реальный текущий)
    ...
    res.json({ ok: true, player, plates: inventory, duels, duels_won, duels_lost, logs });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
function requireAdmin(req, res, next) {
  ...
  if (!admin_id || !isAdmin(admin_id)) {
    return res.status(403).json({ ok: false, error: 'Нет прав администратора' });
  }
  next();
}

// POST /admin/coins — выдать или отобрать монеты
// Body: { admin_id, target_id, delta, reason }
app.post('/admin/coins', requireAdmin, async (req, res) => {
  try {
    const { target_id, delta, reason } = req.body;
    if (!target_id || delta == null) return res.status(400).json({ ok: false, error: 'Нет target_id или delta' });

    const { rows } = await db.query('SELECT coins FROM players WHERE telegram_id=$1', [target_id]);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Игрок не найден' });

    const oldCoins = rows[0].coins;
    const newCoins = Math.max(0, oldCoins + parseInt(delta));

    await db.query('UPDATE players SET coins=$1, updated_at=NOW() WHERE telegram_id=$2', [newCoins, target_id]);

    await db.query(
      `INSERT INTO player_logs (telegram_id, action, details) VALUES ($1,'balance',$2)`,
      [target_id, JSON.stringify({
        old_coins: oldCoins, new_coins: newCoins,
        diff: newCoins - oldCoins, reason: reason || 'admin_action',
        admin_id: req.body.admin_id
      })]
    ).catch(() => {});

    // Уведомить игрока через socket
    emitToPlayer(target_id, 'admin_notify', {
      type: delta > 0 ? 'admin_coins_give' : 'admin_coins_take',
      amount: Math.abs(parseInt(delta))
    });

    res.json({ ok: true, old_coins: oldCoins, new_coins: newCoins });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});

// POST /admin/give_plate — добавить номер в инвентарь игрока
// Body: { admin_id, target_id, plate: { country, vals, rc, text, upgrades, ... } }
app.post('/admin/give_plate', requireAdmin, async (req, res) => {
  try {
    const { target_id, plate } = req.body;
    if (!target_id || !plate) return res.status(400).json({ ok: false, error: 'Нет target_id или plate' });

    const { rows } = await db.query('SELECT inventory FROM players WHERE telegram_id=$1', [target_id]);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Игрок не найден' });

    const inventory = rows[0].inventory || [];
    if (inventory.length >= 10) return res.status(400).json({ ok: false, error: 'Инвентарь игрока полон (10/10)' });

    // Уникальный plate_key
    const plate_key = plate.plate_key || `adm_${Date.now()}_${Math.random().toString(36).slice(2,6)}`;
    const newPlate = { ...plate, plate_key, isAdminGift: true };
    inventory.push(newPlate);

    await db.query(
      'UPDATE players SET inventory=$1, updated_at=NOW() WHERE telegram_id=$2',
      [JSON.stringify(inventory), target_id]
    );

    // Логируем
    await db.query(
      `INSERT INTO player_logs (telegram_id, action, details) VALUES ($1,'admin_give_plate',$2)`,
      [target_id, JSON.stringify({ plate_key, text: plate.text, admin_id: req.body.admin_id })]
    ).catch(() => {});

    // Уведомить через socket
    emitToPlayer(target_id, 'admin_notify', { type: 'admin_plate_give', plate: newPlate });

    res.json({ ok: true, plate_key });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});

// POST /admin/remove_plate — отобрать номер из инвентаря игрока
// Body: { admin_id, target_id, plate_key }
app.post('/admin/remove_plate', requireAdmin, async (req, res) => {
  try {
    const { target_id, plate_key } = req.body;
    if (!target_id || !plate_key) return res.status(400).json({ ok: false, error: 'Нет target_id или plate_key' });

    const { rows } = await db.query('SELECT inventory FROM players WHERE telegram_id=$1', [target_id]);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Игрок не найден' });

    const inventory = rows[0].inventory || [];
    const idx = inventory.findIndex(p =>
      p.plate_key === plate_key ||
      String(p.id) === String(plate_key) ||
      String(p.key) === String(plate_key)
    );
    if (idx === -1) return res.status(404).json({ ok: false, error: 'Номер не найден в инвентаре' });
    ...
    await db.query(
      `INSERT INTO player_logs (telegram_id, action, details) VALUES ($1,'admin_remove_plate',$2)`,
      [target_id, JSON.stringify({ plate_key, text: removed?.text, admin_id: req.body.admin_id })]
    ).catch(() => {});

    emitToPlayer(target_id, 'admin_notify', { type: 'admin_plate_take', plate: removed });

    res.json({ ok: true, removed });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});

// POST /admin/give_upgrade — добавить улучшение в upgInventory игрока
// Body: { admin_id, target_id, upgrade_key }
// Улучшение хранится в players.inventory отдельным полем upgInventory — нет,
// на деле upgInventory лежит в localStorage, но синхронизация идёт через inventory/sync.
// Поэтому добавляем запись в отдельное поле upg_inventory (JSONB), которое клиент сможет подхватить.
app.post('/admin/give_upgrade', requireAdmin, async (req, res) => {
  try {
    const { target_id, upgrade_key } = req.body;
    if (!target_id || !upgrade_key) return res.status(400).json({ ok: false, error: 'Нет target_id или upgrade_key' });

    // Создаём колонку если нет
    await db.query(`ALTER TABLE players ADD COLUMN IF NOT EXISTS upg_inventory JSONB DEFAULT '[]'`).catch(() => {});

    const { rows } = await db.query('SELECT upg_inventory FROM players WHERE telegram_id=$1', [target_id]);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Игрок не найден' });

    const upgInventory = rows[0].upg_inventory || [];
    const newUpg = {
      id: Date.now(),
      lotKey: upgrade_key,
      applied: false,
      ...
      txtColor: '#ffffff',
      frameText: '',
      glowColors: ['#ff0000', '#0000ff', '#00ff00'],
      isAdminGift: true
    };
    upgInventory.push(newUpg);

    await db.query(
      'UPDATE players SET upg_inventory=$1, updated_at=NOW() WHERE telegram_id=$2',
      [JSON.stringify(upgInventory), target_id]
    );

    await db.query(
      `INSERT INTO player_logs (telegram_id, action, details) VALUES ($1,'admin_give_upgrade',$2)`,
      [target_id, JSON.stringify({ upgrade_key, admin_id: req.body.admin_id })]
    ).catch(() => {});

    emitToPlayer(target_id, 'admin_notify', {
      type: 'admin_upgrade_give',
      upgrade: upgrade_key,
      upgrade_name: upgrade_key,
      upg: newUpg
    });

    res.json({ ok: true, upg: newUpg });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});

// POST /admin/remove_upgrade — отобрать улучшение
// Ищем в upg_inventory, затем в upgrades на номерах
app.post('/admin/remove_upgrade', requireAdmin, async (req, res) => {
  try {
    const { target_id, upgrade_key } = req.body;
    if (!target_id || !upgrade_key) return res.status(400).json({ ok: false, error: 'Нет target_id или upgrade_key' });
    ...
    const { rows } = await db.query(
      'SELECT upg_inventory, inventory FROM players WHERE telegram_id=$1', [target_id]
    );
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Игрок не найден' });
    ...
    // 1. Ищем в upg_inventory
    const upgIdx = upgInventory.findIndex(u => u.lotKey === upgrade_key);
    if (upgIdx !== -1) {
      [removed] = upgInventory.splice(upgIdx, 1);
      foundIn = 'upg_inventory';
    }

    // 2. Ищем применённое улучшение на номерах
    if (!removed) {
      for (const plate of inventory) {
        if (plate.upgrades && plate.upgrades[upgrade_key]) {
          plate.upgrades[upgrade_key] = false;
          removed = { lotKey: upgrade_key, appliedToPlate: plate.plate_key };
          foundIn = 'plate_upgrades';
          break;
        }
      }
    }

    if (!removed) return res.status(404).json({ ok: false, error: 'Улучшение не найдено у игрока' });

    if (foundIn === 'upg_inventory') {
      await db.query('UPDATE players SET upg_inventory=$1, updated_at=NOW() WHERE telegram_id=$2',
        [JSON.stringify(upgInventory), target_id]);
    } else {
      await db.query('UPDATE players SET inventory=$1, updated_at=NOW() WHERE telegram_id=$2',
        [JSON.stringify(inventory), target_id]);
    }

    await db.query(
      `INSERT INTO player_logs (telegram_id, action, details) VALUES ($1,'admin_remove_upgrade',$2)`,
      [target_id, JSON.stringify({ upgrade_key, foundIn, admin_id: req.body.admin_id })]
    ).catch(() => {});

    emitToPlayer(target_id, 'admin_notify', {
      type: 'admin_upgrade_take',
      upgrade: upgrade_key,
      upgrade_name: upgrade_key,
      foundIn
    });

    res.json({ ok: true, removed, foundIn });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});

// ── SOCKET.IO ──
// Маппинг telegram_id → socket для личных уведомлений
const playerSockets = new Map(); // telegram_id → Set<socket>

// ID администраторов (добавь свои через .env)
...
// Отправить socket-событие конкретному игроку (по всем его сессиям)
function emitToPlayer(telegram_id, event, data) {
  const sockets = playerSockets.get(String(telegram_id));
  if (!sockets) return;
...
io.on('connection', (socket) => {
  // Игрок регистрирует свой telegram_id для личных уведомлений
  socket.on('identify', ({ telegram_id }) => {
    if (!telegram_id) return;
    const tid = String(telegram_id);
    socket.telegram_id = tid;
    if (!playerSockets.has(tid)) playerSockets.set(tid, new Set());
    playerSockets.get(tid).add(socket);
  });

  // Отключение — убираем из маппинга
  socket.on('disconnect', () => {
    if (socket.telegram_id) {
      const set = playerSockets.get(socket.telegram_id);
      if (set) {
        set.delete(socket);
        if (set.size === 0) playerSockets.delete(socket.telegram_id);
      }
    }
  });

  // Прокси: клиент-админ шлёт admin_notify → сервер доставляет целевому игроку
  socket.on('admin_notify', ({ target_id, type, amount, plate, upgrade_name }) => {
    if (!socket.telegram_id || !isAdmin(socket.telegram_id)) return;
    emitToPlayer(target_id, 'admin_notify', { type, amount, plate, upgrade_name });
  });
